	...
	if (!keystone.get('cloudinary config')) {
		throw new Error(
			'Invalid Configuration\n\n' +
				'CloudinaryImages fields (' +
				list.key +
				'.' +
				this.path +
				') require the "cloudinary config" option to be set.\n\n' +
...
cloudinaryimages.prototype.addToSchema = function(schema) {
	...
	var folder = function(item) {
		...
		if (keystone.get('cloudinary folders')) {
			if (field.options.folder) {
				...
				var folderList = keystone.get('cloudinary prefix')
	...
	ImageSchema.method('fill', function(width, height, options) {
		return src(
			this,
			addSize({ crop: 'fill', gravity: 'faces' }, width, height, options)
		);
	});
	ImageSchema.method('lfill', function(width, height, options) {
		return src(
			this,
			addSize({ crop: 'lfill', gravity: 'faces' }, width, height, options)
		);
	});
	...
	ImageSchema.method('crop', function(width, height, options) {
		return src(
			this,
			addSize({ crop: 'crop', gravity: 'faces' }, width, height, options)
		);
	});
	ImageSchema.method('thumbnail', function(width, height, options) {
		return src(
			this,
			addSize({ crop: 'thumb', gravity: 'faces' }, width, height, options)
		);
	});
...
cloudinaryimages.prototype._originalGetOptions =
	cloudinaryimages.prototype.getOptions;
	...
	async.map(
		...
		function(value, next) {
			if (typeof value === 'object' && 'public_id' in value) {
				...
				if (
					field.options.filenameAsPublicID &&
					value.originalname &&
					typeof value.originalname === 'string'
				) {
					uploadOptions = assign({}, uploadOptions, {
						public_id: value.originalname.substring(
							0,
